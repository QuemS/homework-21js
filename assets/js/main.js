    ...
    let arrPeriods = [];                                       //array with the number of frosts in days
    
    for( i = 0; i < winterDays.length - 1; i++){
        if ( (winterDays[i] >= 0 ) && (winterDays[i + 1] < 0 )) {
            let number = 0;                                   //Days counter
            for(let j = i + 1; j < winterDays.length ; j++){
                if ((winterDays[j] < 0 ) ){
                    number++;
                }else{
                    arrPeriods.push(number)
                    break;
                }
    ...
    let maxPeriods = arrPeriods[0];                              // Maximum freezing period in days
    let amountFrost = arrPeriods.length;                         // number of freezes 
